...
function zeroBSCRM_updateTransStatus($id, $status, $old_status){
	...
	$orderID = (int)zeroBSCRM_giveWP_getTransactionID_byMeta('extra_zbs_givewp_uid', $purchase_key);

	if ($orderID > 0){

		// update status
		$zbs->DAL->transactions->setTransactionStatus($orderID,$zbt['status']);
...
function zeroBSCRM_storeGiveWPDonar( $args, $payment_data ) {
	...
	if ($isZBSAddition && $fieldIndexes['name'] !== -1 && $fieldIndexes['email'] !== -1){
		...
		if (!empty($customerEmail)){
		    	...
		    	#} Add tag
				global $zbs;
				if ($custID > 0) $zbs->DAL->contacts->addUpdateContactTags(array(
	            	
	            	'id'            => $custID,
	            	'tags'          => array(__('GiveWP','zero-bs-crm')),
	            	'mode'			=> 'append'));
			...
			$orderID = zeroBSCRM_giveWP_getTransactionID_byMeta('zbs_givewp_uid', $payment_data['purchase_key']);
			if (!$orderID && isset($custID) && !empty($custID)){

				$date = date("U");
				#} payment_data['date'] is empty. Use current date.
				if(array_key_exists('date', $payment_data)){
				...
				$zbo = array(
	                ...
	                'date_completed' => '',
	                'lineitems'     => false, 
	                'contacts' => array($custID), // array of id's
				...
				// add transaction
	            $transactionID = $zbs->DAL->transactions->addUpdateTransaction(array(
	            	
	            	'data'      => $zbo,
	            	'extraMeta' => array(
	            		'zbs_givewp_uid' => $payment_data['purchase_key']
	            	)
	            ...
	            if ($transactionID > 0){
	            	...
	            	// add tags
					$zbs->DAL->transactions->addUpdateTransactionTags(array('id'=>$transactionID,'tags'=>array(__('GiveWP','zero-bs-crm')),'mode'=>'append'));

				}

		} // / if customer, add trans 
...
// previously zeroBSCRM_giveWP_getPostID_byMeta
// retrieves a transaction ID which has a matching meta
function zeroBSCRM_giveWP_getTransactionID_byMeta($key, $value) {

	global $zbs;

	return $zbs->DAL->getIDWithMeta(array(
            'objtype'       => 5, // 5 = transaction
            'key'   => $key,
            'val'   => $value
        ));
